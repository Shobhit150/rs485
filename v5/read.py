...
PARAMS = {
    ...
    206: ("Tariff EB",             0.01,    "Rs",     "tariff"),
    207: ("Tariff DG",             0.01,    "Rs",     "tariff"),
    ...
    215: ("Neg Balance Limit",     1,       "Rs",     "swap16"),
    216: ("Balance",               0.001,   "Rs",     None),
    217: ("Tamper",                1,       "",       None),
    218: ("Happy Hour",            1,       "",       "hour"),
    219: ("Happy Day",             1,       "",       "day"),
    ...
    226: ("Monthly Tariff",        1,       "Rs",     "monthly"),
    ...
    263: ("10th Deduction",        0.01,    "Rs",     "debug"),
...
def display(sno, raw):
    ...
    if special == "tariff":
        # bytes within lower 16 are swapped: 0x7805 → swap → 0x0578 = 1400 → 14.00 Rs
        lo16 = raw & 0xFFFF
        swapped = ((lo16 & 0xFF) << 8) | ((lo16 >> 8) & 0xFF)
        if swapped in (0xFFFF, 0xFFFE):
            return raw, "⚠️  NOT SET"
        return raw, f"{swapped * scale:.2f} {unit}"

    if special == "hour":
        # 0x1112 → start=0x11=17, end=0x12=18
        if raw in (0xFFFF, 0xFFFFFFFF):
            return raw, "⚠️  NOT SET"
        start = (raw >> 8) & 0xFF
        end   = raw & 0xFF
        return raw, f"{start:02d}:00 – {end:02d}:00"

    if special == "day":
        # 0x0A0C → start_day=10, end_day=12
        if raw in (0xFFFF, 0xFFFFFFFF):
            return raw, "⚠️  NOT SET"
        start = (raw >> 8) & 0xFF
        end   = raw & 0xFF
        return raw, f"day {start} to {end}"

    if special == "monthly":
        # This register is a composite — the monthly tariff value (0x78=120)
        # written to B1CA does not cleanly appear in this 32-bit read slot.
        # Show upper 16 bits as the setting value.
        val = (raw >> 16) & 0xFFFF
        if val == 0:
            val = (raw >> 8) & 0xFF  # try byte 2
        return raw, f"{val} Rs"
    ...
    if special == "debug":
        # Last register in block — upper 16 bits is actual value, lower 16 is next register bleed
        val = (raw >> 16) & 0xFFFF
        return raw, f"{val * scale:.2f} {unit}"
